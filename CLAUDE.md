...
- `memcore server start` warns on stderr that the REST API is **UNAUTHENTICATED** whenever `MNEM_API_KEY` is unset and the host isn't loopback. The container binds 0.0.0.0, so a key has been set since 2026-09-25.
...
- **Auth**: with `MNEM_API_KEY` set, every route except `GET /health` and `GET /livez` needs `Authorization: Bearer <key>` or `X-API-Key: <key>`. Unset keeps the API open. **Enabled on SkyNAS 2026-09-25 ~05:40** (container recreated with `--no-build`, volume untouched): no key / wrong key → 401, key → 200; the Windows bridge and `.mcp.json` send it. The MCP bridge (`server mcp --remote`) sends `MNEM_API_KEY` / `--api-key` from its own environment.
